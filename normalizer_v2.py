"""
🔧 Módulo de Normalización de Vehículos v6.2
=============================================

CAMBIOS v6.2 vs v6.1:
──────────────────────
- _fallback_modelo: filtra motorizaciones y trims antes de
  aceptar una palabra como modelo
- _fallback_modelo: busca modelos de catálogo en descripción
  antes de rendirse
- _find_modelo: estrategia descripción ANTES de nombre base
- _find_modelo: nombre base filtra MOTOR_SUFFIX_TOKENS
- Post-check: si modelo es motorización/trim → re-buscar
- _is_motorization ampliado con más patrones argentinos
- _find_version: busca trim puro, version = trim (no string
  completo)
- Motor y transmisión se devuelven en campos separados
- Paso 3.5 ELIMINADO (no expande modelo con trim)
"""
...
class NormalizationStats:
    ...
    modelo_was_motor: int = 0
    modelo_was_trim: int = 0
    by_method: Dict[str, int] = field(
        default_factory=lambda: {
            'exact': 0, 'alias': 0, 'code_mapping': 0,
            'base_name': 0, 'description': 0,
            'version_split': 0, 'fuzzy': 0, 'fallback': 0,
        }
    )
    ...
    def summary(self) -> str:
        ...
        return (
            ...
            f"({p(self.partial_match)}) | "
            f"Fallback: {self.fallback_used} "
            f"({p(self.fallback_used)}) | "
            f"MotorAsModelo: {self.modelo_was_motor} | "
            f"TrimAsModelo: {self.modelo_was_trim}"
        )
...
TRANS_TOKENS = frozenset({
    'at', 'mt', 'cvt',
    'at4', 'at5', 'at6', 'at7', 'at8', 'at9', 'at10',
    'mt5', 'mt6', 'mt7',
    'dsg', 'dct', 'bva', 'edc', 'amt', 'ddct',
    'tiptronic', 'sportronic', 'lineartronic', 'powershift',
    'stronic', 's-tronic', 'pdk', 'smg', 'mct', 'tct',
    'steptronic', 'xtronic', 'x-tronic',
    'ecvt', 'e-cvt', 'geartronic', 'ivt',
    'eat6', 'eat8',
    '9g-tronic', '7g-tronic', '8g-tronic',
...
DOOR_TOKENS = frozenset({
    '2p', '3p', '4p', '5p',
    '2ptas', '3ptas', '4ptas', '5ptas',
})

BODY_TOKENS = frozenset({
    'cabina', 'chasis', 'doble', 'simple', 'extendida',
    'pickup', 'pick-up',
    'sedan', 'hatchback', 'hatch', 'berlina',
    'liftback', 'fastback', 'sportback', 'notchback',
    'rural', 'sw', 'wagon', 'familiar', 'estate', 'break',
    'touring', 'tourer', 'avant', 'variant', 'sportwagon',
    'alltrack', 'allroad',
    'van', 'furgon', 'furgoneta', 'panel',
    'minivan', 'monovolumen', 'multispace', 'spacetourer',
    'coupe', 'convertible', 'cabriolet', 'cabrio',
    'roadster', 'spider', 'spyder', 'targa', 'speedster',
    'suv',
})

EQUIPMENT_TOKENS = frozenset({
    'cuero', 'tela', 'vinilo', 'alcantara',
    'full', 'semifull',
    'gnc', 'glp', 'gps', 'navegador', 'navi',
    'xenon', 'bixenon', 'led',
    'sunroof', 'moonroof', 'panoramico',
    'bluetooth', 'multimedia',
    'abs', 'esp', 'climatizador', 'climatronic',
    'camara', 'sensores', 'parking', 'keyless',
    'airbag', 'airbags', 'blindado',
})

MOTOR_SUFFIX_TOKENS = frozenset({
    't', 'td', 'turbo', 'biturbo', 'twinturbo', 'twinpower',
    'supercharged', 'kompressor', 'intercooler',
    'tdi', 'tfsi', 'tsi', 'fsi', 'sdi',
    'hdi', 'bluehdi', 'vti', 'thp', 'puretech',
    'mpi', 'gdi', 'tgdi', 't-gdi',
    'ts', 'jts', 'jtd', 'jtdm',
    'cdti', 'cdi', 'cgi', 'dci', 'tce', 'sce',
    'crdi', 'tdci', 'ctdi',
    'mjet', 'multijet',
    'ecoboost', 'ecoblue', 'duratec', 'zetec',
    'ecotec', 'duramax', 'vortec', 'flexpower',
    'multiair', 'twinair', 'fire', 'firefly',
    'etorq', 'e.torq',
    'vtec', 'i-vtec', 'ivtec', 'i-dtec', 'idtec',
    'skyactiv', 'skyactiv-g', 'skyactiv-d', 'skyactiv-x',
    'mivec', 'di-d', 'did', 'valvetronic',
    'panther', 'puma', 'mwm', 'maxion', 'perkins',
    'cummins', 'sprint', 'energy',
    'boxer', 'flat', 'vvt-i', 'vvti', 'd-4d', 'd4d',
    'diesel', 'nafta', 'naftero', 'gasolina',
    'aspirado', 'atmosferico',
    'hev', 'phev', 'mhev', 'fhev', 'ev', 'bev', 'fcev',
    'epower', 'e-power', 'e-tech', 'etech',
    'hybrid', 'hibrido', 'e:hev', 'bluetec',
    'plugin', 'plug-in', 'lfp',
    'v4', 'v6', 'v8', 'v10', 'v12',
    'l3', 'l4', 'l5', 'l6',
    'i3', 'i4', 'i6', 'w12', 'w16',
...
BARE_NUMBER_MOTOR_TRIGGERS = frozenset({
    'tsi', 'tfsi', 'tdi', 'fsi', 'mpi', 'gdi', 'tgdi',
    'cv', 'hp', 'bhp', 'ps',
    'turbo', 'diesel', 'amg', 'kompressor',
    'hdi', 'bluehdi', 'dci', 'tce',
    'ecoboost', 'ecoblue', 'mjet', 'multijet',
    'jtd', 'jtdm', 'crdi', 'cdti', 'cdi', 'tdci',
    'vtec', 'mivec',
})

# Tokens que NUNCA son modelos (motorizaciones cortas)
NEVER_MODEL_TOKENS = frozenset({
    'hdi', 'tdi', 'tsi', 'tfsi', 'thp', 'vti', 'fsi',
    'mpi', 'gdi', 'crdi', 'cdti', 'cdi', 'dci', 'tce',
    'sce', 'mjet', 'jtd', 'jtdm', 'tdci', 'ctdi',
    'ecoboost', 'ecoblue', 'ecotec', 'multijet',
    'puretech', 'bluehdi', 'firefly', 'fire',
    'vtec', 'mivec', 'skyactiv', 'duratec', 'zetec',
    'turbo', 'diesel', 'nafta', 'naftero',
    'automatico', 'automatica', 'manual',
    'at', 'mt', 'cvt', 'dsg', 'tiptronic',
}) | TRANS_TOKENS

TRANS_CANONICAL = {
    'automatica': 'at', 'automatico': 'at',
    'automatic': 'at', 'manual': 'mt',
    'secuencial': 'at', 'sequential': 'at',
    'tiptronic': 'at', 'sportronic': 'at',
    'steptronic': 'at', 'lineartronic': 'cvt',
    'xtronic': 'cvt', 'x-tronic': 'cvt',
    'e-cvt': 'cvt', 'ecvt': 'cvt',
    'dsg': 'at', 'dct': 'at', 'ddct': 'at',
    's-tronic': 'at', 'stronic': 'at',
    'pdk': 'at', 'smg': 'at', 'amt': 'at',
    'bva': 'at', 'powershift': 'at',
    'geartronic': 'at',
}
...
DEFAULT_BRAND_ALIASES = {
    ...
    'mb': 'mercedes-benz', 'benz': 'mercedes-benz',
...
FUZZY_BLACKLIST = {
    ...
    'hibrido', 'electrico',
    'manual', 'automatico', 'automatica', 'secuencial',
    'cvt', 'dsg',
    'muy', 'buen', 'bueno', 'buena', 'mal', 'estado',
    'impecable', 'excelente', 'perfecto', 'perfecta',
    'inmaculado', 'nuevo', 'nueva', 'usado',
    'unico', 'unica', 'dueno', 'dueño', 'titular',
    'particular', 'full', 'tope', 'gama', 'equipo',
    'equipado', 'completo', 'cuero', 'techo',
    'permuto', 'permuta', 'financio', 'vendo', 'venta',
    'contado', 'auto', 'autos', 'coche', 'vehiculo',
    'carro', 'motor', 'caja', 'cambios', 'vtv', 'papeles',
    'patente', 'seguro', 'service', 'consultar', 'precio',
    'oferta', 'urgente', 'oportunidad', 'original',
    'funcionando', 'kilometros', 'kms', 'litros',
    'puertas', 'modelo', 'version', 'serie', 'linea',
    'tipo', 'funciona', 'todo', 'anda', 'tiene', 'soy',
    'directo', 'transferencia', 'pesos', 'dolares', 'usd',
    'efectivo',
}
...
MOTORIZATION_PATTERNS = [
    r'^\d+\.\d+$',
    r'^\d+\.\d+\s*[lt]?$',
    r'^\d+\.\d+\s*(tdi|tfsi|hdi|vti|thp|jtd|cdti|dci|'
    r'tsi|fsi|mpi|ts|jts|jtdm|crdi)i?$',
    r'^\d+\.\d+\s*(turbo|diesel|nafta|naftero|multijet|'
    r'bluehdi|ecoboost)$',
    r'^\d+(v|cv|hp|bhp)$',
    ...
    r'^\d+\.\d+\s+\d+v$',
    r'^\d+v$',
    # v6.2: tecnologías sueltas
    r'^(hdi|tdi|tsi|tfsi|thp|vti|fsi|mpi|gdi|crdi|cdti|'
    r'cdi|dci|tce|mjet|jtd|tdci|ecoboost|bluehdi|'
    r'puretech|multijet|ecotec)$',
    r'^(hdi|tdi|tsi|tfsi|thp|vti)\s+\d+',
]

URGENCY_KEYWORDS = {
    'urgente': 30, 'urge': 30, 'urgencia': 30,
    'viajo': 25, 'viaje': 25, 'me voy': 25,
    'mudanza': 25, 'oportunidad': 20, 'negociable': 15,
    'escucho': 15, 'escucho ofertas': 20,
    'acepto oferta': 20, 'vendo ya': 20,
    'venta rapida': 20, 'liquido': 20,
...
# ═══════════════════════════════════════════════════════════════
# 🔩 DESCOMPOSICIÓN DE INPUT
# ═══════════════════════════════════════════════════════════════

def _is_year_phase_token(token: str) -> bool:
    t = token.lower()
    if t in (
        'linea', 'fase', 'phase', 'facelift',
        'restyling', 'lci',
    ):
        return True
    return bool(re.match(
        r'^(my|l|mk|g|ph|gen|fase|phase)\.?\d{1,4}(\.\d)?$',
        t,
    ))
...
def _is_motor_start(
    ...
    if re.match(r'^t\d{3}$', t):
        return True
    if t in (
        'ev', 'bev', 'phev', 'hev', 'mhev', 'fhev', 'fcev',
        'e-power', 'epower', 'e-tech', 'etech',
        'hybrid', 'hibrido',
    ):
        return True
    if t in (
        'v6', 'v8', 'v10', 'v12', 'l3', 'l4', 'l6',
        'i3', 'i4', 'i6', 'w12', 'w16',
    ):
        return True
    if t in (
        'skyactiv', 'skyactiv-g', 'skyactiv-d', 'skyactiv-x',
        'ecoboost', 'ecoblue', 'ecotec', 'duramax',
        'multijet', 'multiair', 'twinair', 'boxer',
        'puretech', 'bluehdi', 'firefly', 'fire',
        'vtec', 'i-vtec', 'mivec',
    ):
        return True
...
def decompose_user_input(text: str) -> Dict[str, Any]:
    result = {
        ...
        'traccion': None, 'puertas': None,
        'carroceria': None, 'tiene_gnc': None,
        'body_tokens': [], 'equipment_tokens': [],
    }
    if not text:
        return result
    ...
    if re.search(r'\bs(?:in)?[/\s]gnc\b', t_lower):
        ...
        r'\bgnc\b|\bgas\s*natural\b|\bc(?:on)?[/\s]gnc\b',
        t_lower,
    ):
    ...
    motor_parts, trim_parts = [], []
    body_parts, equip_parts = [], []
    i = 0
    ...
    while i < len(tokens):
        ...
        if _is_motor_start(t, next_t):
            ...
            i += 1
            while (i < len(tokens)
                   and tokens[i] in MOTOR_SUFFIX_TOKENS):
...
def _motors_compatible(
    cat_motor: str, user_motor: str
) -> bool:
    ...
    return (
        cat_motor in user_motor
        or user_motor in cat_motor
    )


def _trans_compatible(
    cat_trans: str, user_trans: str
) -> bool:
...
def _normalize_motor_canonical(
    motor: str, aliases: Dict = None
) -> str:
    if not motor:
        return ''
    ...
    if m2 != m:
        return m2
    m2 = re.sub(
        r'^(\d+\.\d+)\s+turbo\s*diesel$', r'\1td', m
    )
    if m2 != m:
        return m2
    return m
...
def _normalize_trans_canonical(
    trans: str, aliases: Dict = None
) -> str:
    if not trans:
        return ''
...
def _detect_0km(km, text: str) -> bool:
    if km is not None and km == 0 and text:
        return bool(re.search(
            r'\b0\s*km\b|\bcero\s*km\b|\bokm\b',
            text.lower(),
        ))
    return False
...
def extract_year_from_text(text: str) -> Optional[int]:
    ...
    years = [
        ...
        if int(y) <= CONFIG.year_max_valid
    ]
    return max(years) if years else None


def extract_year_from_description(
    text: str
) -> Optional[int]:
    if not text:
        return None
...
def assess_year_quality(
    año_structured=None, año_description=None,
    km=None, model_year_range=None
):
    result = {
        'quality': 'valid', 'best_year': año_structured,
        'year_source': 'structured', 'reason': None,
    }
    if not año_structured:
        if año_description:
            result.update(
                ...
                reason='Año de descripción',
            )
        else:
            result.update(
                quality='missing', reason='Sin año',
            )
        return result
    ...
    if año_structured > CONFIG.year_max_valid:
        ...
        if (año_description
                and CONFIG.year_min_valid
                <= año_description
                <= CONFIG.year_max_valid):
            result.update(
                best_year=año_description,
                year_source='description',
            )
    ...
    if año_structured < CONFIG.year_min_valid:
        result['quality'] = 'garbage'
        result['reason'] = (
            f"Año {año_structured} < {CONFIG.year_min_valid}"
        )
        result['best_year'] = (
            año_description
            if (año_description
    ...
    if (km and km > CONFIG.km_max_for_current_year
        ...
        result.update(
            quality='suspicious',
            reason=(
                f"Año {año_structured} con {km:,} km"
            ),
        )
        if (año_description
                and año_description < año_structured):
            result.update(
                best_year=año_description,
                year_source='description',
            )
    ...
    if model_year_range:
        ...
        if dist > CONFIG.year_garbage_threshold:
            result['quality'] = 'garbage'
            result['reason'] = (
                f"Año {año_structured} a {dist} "
                f"años de {desde}-{hasta}"
            )
            if (año_description
                    and CONFIG.year_min_valid
                    <= año_description
    ...
    if (result['quality'] == 'valid'
        ...
        result.update(
            quality='suspicious',
            reason=(
                f"Año struct {año_structured} ≠ "
                f"desc {año_description}"
            ),
        )
    return result
...
def extract_model_from_description(
    descripcion, marca, catalog_models
):
...
def split_version_into_model_and_trim(
    version_raw, catalog_models, marca=""
):
...
class EnrichedCatalog:
    ...
    def load(
        ...
        try:
            ...
            if (brand_aliases_path
                    and os.path.exists(brand_aliases_path)):
                with open(
                    brand_aliases_path, 'r', encoding='utf-8'
                ) as f:
                    extra = json.load(f)
                for alias, brand in extra.get(
                    'brand_aliases', {}
                ).items():
                    self._brand_aliases[
                        normalize_key(alias)
            ...
            logger.info(
                f"✅ Catálogo "
                f"v{meta.get('generator_version', '?')}: "
    ...
    def get_brands(self):
        return sorted(
            self.catalog_index.keys(), key=len, reverse=True
        )

    def get_models(self, brand):
        return sorted(
            self.catalog_index.get(
                normalize_key(brand), {}
            ).keys(),
    ...
    def has_model(self, brand, model):
        return (
            normalize_key(model) in self.catalog_index.get(
                normalize_key(brand), {}
            )
        )
    ...
    def get_brand_trims(self, brand) -> Set[str]:
        """Retorna TODOS los trims de una marca."""
        brand_norm = normalize_key(brand)
        trim_idx = self.trim_index.get(brand_norm, {})
        return set(trim_idx.keys())

    def get_version_components_safe(
        self, brand, model
    ) -> Dict:
        key = make_key(brand, model)
    ...
    def get_known_trims_safe(
        ...
        components = self.get_version_components_safe(
            brand, model
        )
        trim_set = set()
    ...
    def get_confusions(self, brand, model):
        return self.known_confusions.get(
            make_key(brand, model), []
        )
...
class VehicleNormalizerV4:
    ...
    def normalize(
        ...
        año_raw=None, precio_raw=None, km_raw=None
    ) -> Dict:
        global STATS
        STATS.total += 1
        ...
        result = {
            ...
            'version': None, 'motor': None,
            'transmision_normalizada': None,
            'año_detectado': None, 'año_fuente': None,
            'año_sospechoso': False,
            'norm_status': 'pending',
            'from_catalog': False,
            'match_method': None,
            'confidence': 0, 'confidence_level': 'very_low',
            'version_method': None, 'version_score': 0,
            'extracted_data': {
                'puertas': None, 'traccion': None,
                'carroceria': None, 'tiene_gnc': None,
                'es_0km': False,
            },
        ...
        año_q = assess_year_quality(
            ...
            año_description=año_desc, km=km_raw,
        )
        año = año_q['best_year']
        ...
        modelo_result = self._find_modelo(
            marca=marca, titulo=titulo_n,
            modelo_raw=modelo_n, version_raw=version_n,
            descripcion=desc_n,
            text_high=text_high,
        ...
        # ═══ PASO 3.5: POST-CHECK MODELO ═══
        # Si el modelo es una motorización o trim conocido,
        # re-buscar en descripción
        if (result['modelo']
                and result['marca']
                and not result['from_catalog']):
            modelo_check = normalize_key(result['modelo'])
            is_motor = (
                modelo_check in NEVER_MODEL_TOKENS
                or is_motorization_pattern(modelo_check)
            )
            brand_trims = self.catalog.get_brand_trims(marca)
            is_trim = modelo_check in brand_trims

            if is_motor or is_trim:
                if is_motor:
                    STATS.modelo_was_motor += 1
                    result['warnings'].append(
                        f"'{result['modelo']}' es "
                        f"motorización, no modelo"
                    )
                else:
                    STATS.modelo_was_trim += 1
                    result['warnings'].append(
                        f"'{result['modelo']}' es "
                        f"trim, no modelo"
                    )
                ...
                found = None
                if desc_n and modelos:
                    found = extract_model_from_description(
                        desc_n, marca, modelos
                    )
                if not found and titulo_n and modelos:
                    found = extract_model_from_description(
                        titulo_n, marca, modelos
                    )

                if found:
                    result['modelo'] = found
                    result['from_catalog'] = True
                    result['match_method'] = 'description'
                    STATS.model_from_description += 1
                else:
                    result['modelo'] = None
                    result['from_catalog'] = False
    ...
    # ── MODELO (v6.2: orden corregido + filtros) ──
    def _find_modelo(
        ...
        if not modelos:
            return self._fallback_modelo(
                modelo_raw, version_raw, titulo,
                marca, descripcion,
            )

        combined = (
            f"{version_raw} {modelo_raw} "
            f"{titulo} {text_all}"
        )
        ...
        # 4. DESCRIPCIÓN (SUBIDO de 5 a 4 en v6.2)
        if descripcion:
        ...
        # 5. Nombre base (BAJADO de 4 a 5, CON filtro)
        brand_trims = self.catalog.get_brand_trims(marca)
    ...
    def _fallback_modelo(
        self, modelo_raw, version_raw, titulo,
        marca, descripcion=""
    ):
        """
        v6.2: Busca modelo en descripción antes de tomar
        primera palabra. Filtra motorizaciones y trims.
        """
        # Intentar descripción primero
        if descripcion and self.catalog.is_loaded:
            modelos = self.catalog.get_models(marca)
            if modelos:
                found = extract_model_from_description(
                    descripcion, marca, modelos
        ...
        # Intentar título
        if titulo and self.catalog.is_loaded:
            modelos = self.catalog.get_models(marca)
            if modelos:
                found = extract_model_from_description(
                    titulo, marca, modelos
                )
                if found:
                    STATS.model_from_description += 1
                    return {
        ...
        combined = (
            f"{version_raw} {modelo_raw} {titulo}"
        )
        if marca:
        ...
        brand_trims = (
            self.catalog.get_brand_trims(marca)
        ...
        for w in combined.split():
            ...
            if (len(w) >= 2
                    ...
                    and w_lower not in FUZZY_BLACKLIST
                    and w_lower not in NEVER_MODEL_TOKENS
                    and w_lower not in brand_trims
                    and not is_motorization_pattern(w_lower)):
                return {
                    'value': w_lower,
                    'from_catalog': False,
                    'method': 'fallback',
    ...
    # ── VERSIÓN (v6.2: busca trim puro) ──
    def _find_version(
        self, marca, modelo, version_raw, search_text
    ) -> Optional[Dict]:
        trims = self.catalog.get_trims(marca, modelo)
        versiones = self.catalog.get_versions(marca, modelo)
        combos = self.catalog.get_valid_combos(
            marca, modelo
        )
        ...
        if user_trim:
            for trim in trims_sorted:
                tl = trim.lower()
                if (user_trim == tl
                        or user_trim.startswith(tl + ' ')
                        or user_trim.endswith(' ' + tl)
                        or f' {tl} ' in f' {user_trim} '):
        ...
        # 3. Match exacto versión completa → extraer trim
        ...
        # 4. Component scoring (solo con trim)
        if user_trim and combos:
            ...
            for combo in combos:
                ct = (combo.get('trim') or '').lower()
                cm = (combo.get('motor') or '').lower()
                ctr = (combo.get('trans') or '').lower()

                score = 0
                if ct and user_trim:
                    if ct == user_trim:
                        score += 60
                    elif (user_trim in ct
                          or ct in user_trim):
                        score += 45
                    else:
                        continue

                if cm and user_motor:
                    if _motors_compatible(cm, user_motor):
                        score += 25
                elif not cm and not user_motor:
                    score += 10

                if ctr and user_trans:
                    if _trans_compatible(ctr, user_trans):
                        score += 15
                elif not ctr and not user_trans:
                    score += 5
        ...
        if CONFIG.enable_fuzzy and user_trim and trims:
            match = fuzzy_match(user_trim, trims, 80)
            if match:
                return {
                    'value': match[0],
                    'from_catalog': True,
    ...
    # ── Helpers de versión ──
    def _clean_version_input(
        self, version_raw, marca, modelo
    ):
        text = (
        ...
        if not text:
            return ""
        if marca:
            text = _safe_re_remove(
                text, normalize_key(marca)
            )
            marca_nh = (
                normalize_key(marca).replace('-', ' ')
            )
        ...
        if modelo:
            text = _safe_re_remove(
                text, normalize_key(modelo)
            )
        return ' '.join(text.split())

    def _decompose_and_normalize(
        self, clean_input, search_text=""
    ):
        comp = decompose_user_input(clean_input or "")
        if (not comp.get('trim')
                and not comp.get('motor')
                and search_text):
            comp2 = decompose_user_input(
                search_text[:200]
            )
    ...
    def _post_validate(self, result, marca, año):
        ...
        confusions = self.catalog.get_confusions(
            marca, result['modelo']
        )
        if confusions:
            names = [
                c.get('modelo_confundido', '?')
                for c in confusions
            ]
            result['warnings'].append(
    ...
    def _try_year_correction(
        ...
        sug = self.catalog.get_year_suggestion(
            marca, modelo, año
        )
        ...
        if año > hasta:
            ...
            self._walk_chain(
                result, marca, modelo, año,
                desde, hasta, 'predecesor',
            )

    def _walk_chain(
        self, result, marca, modelo, año,
    ...
    def _apply_year_correction(
        self, result, old, new, od, oh, nd, nh, año
    ):
        result['corrections'].append({
            'tipo': 'year_mismatch',
            'original': old, 'corregido': new,
            'razon': (
                f"'{old}' {od}-{oh}, "
                f"año {año} → '{new}' {nd}-{nh}"
            ),
...
def init_normalizer(
    dicts_path=None, brand_aliases_path=None
) -> bool:
    global CATALOG, NORMALIZER, STATS
    STATS = NormalizationStats()
    dicts_path = dicts_path or CONFIG.dicts_path
    brand_aliases_path = (
        brand_aliases_path or CONFIG.brand_aliases_path
    )
